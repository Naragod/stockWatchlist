...
```commandline
curl https://app.albert.com/casestudy/stock/prices/?tickers=APPL,MSFT -H "Albert-Case-Study-API-Token: <token>"
...
```commandline
curl --location 'http://localhost:8000/login/' --header 'Content-Type: application/json' --data '{"username": "user1"}'
...
- `make users` - this will create a superuser for the Django admin page with username=root and password=root
